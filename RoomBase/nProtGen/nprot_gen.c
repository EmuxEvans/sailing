...
static int generate_hfile(const char* name, char* inc, unsigned int inc_len, const char flag);
static int generate_cfile(const char* name, char* src, unsigned int src_len, const char flag);
...
int main(int argc, char* argv[])
{
	...
	memset(txt, 0, sizeof(txt));
	ret = generate_hfile(argv[2], txt, sizeof(txt), 'C');
	if(ret!=ERR_NOERROR) {
		printf("error: parse!\n");
		return 0;
	}
	...
	memset(txt, 0, sizeof(txt));
	ret = generate_cfile(argv[2], txt, sizeof(txt), 'C');
	if(ret!=ERR_NOERROR) {
		printf("error: parse!\n");
		return 0;
	}
	...
	memset(txt, 0, sizeof(txt));
	ret = generate_hfile(argv[2], txt, sizeof(txt), 'S');
	if(ret!=ERR_NOERROR) {
		printf("error: parse!\n");
		return 0;
	}
	...
	memset(txt, 0, sizeof(txt));
	ret = generate_cfile(argv[2], txt, sizeof(txt), 'S');
	if(ret!=ERR_NOERROR) {
		printf("error: parse!\n");
		return 0;
	}
...
int generate_hfile(const char* name, char* inc, unsigned int inc_len, const char flag)
{
	struct tm   *newTime;
    time_t      szClock;
	int c, f, p;
	...
	pname = (flag=='C'?"Client":"Server");
	kname = (flag!='C'?"Client":"Server");
	inc[0] = '\0';
    time(&szClock);
    newTime = localtime(&szClock);
	...
	snprintf(inc+strlen(inc), inc_len-strlen(inc), "#ifndef __%s_%s_INCLUDE__\n", buf, flag=='C'?"CLIENT":"SERVER");
	snprintf(inc+strlen(inc), inc_len-strlen(inc), "#define __%s_%s_INCLUDE__\n", buf, flag=='C'?"CLIENT":"SERVER");
	snprintf(inc+strlen(inc), inc_len-strlen(inc), "\n");

	for(c=0; c<nmodules_count; c++) {
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "class C%s%sStub : public I%s%s {\n", nmodules[c].name, pname, nmodules[c].name, kname);
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "public:\n");
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "	C%s%sStub(CDataStream* pStream) { m_pStream = pStream; }\n", nmodules[c].name, pname);
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "\n");
		for(f=nmodules[c].f_start; f<nmodules[c].f_start+nmodules[c].f_count; f++) {
			if(nfunctions[f].type==flag) continue;
		...
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "	CDataStream* GetDataStream() { return m_pStream; }\n");
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "private:\n");
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "	CDataStream* m_pStream;\n");
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "};\n");
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "class C%s%sParser {\n", nmodules[c].name, pname);
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "public:\n");
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "	C%s%sParser(CDataStream* pStream, I%s%s** pHooks, int nHookMax) {\n", nmodules[c].name, pname, nmodules[c].name, pname);
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "		m_pStream = pStream;\n");
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "		m_pHooks = pHooks;\n");
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "		m_nHookMax = nHookMax;\n");
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "	}\n");
		//snprintf(inc+strlen(inc), inc_len-strlen(inc), "	~C%s%sParser() {\n", nmodules[c].name, pname);
		//snprintf(inc+strlen(inc), inc_len-strlen(inc), "	}\n");
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "\n");
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "	bool Dispatch(const void* pData, unsigned int nSize);\n", nmodules[c].name);
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "\n");
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "private:\n");
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "	CDataStream* m_pStream;\n", nmodules[c].name, kname);
		snprintf(inc+strlen(inc), inc_len-strlen(inc), "	I%s%s** m_pHooks;\n", nmodules[c].name, pname);
...
int generate_cfile(const char* name, char* src, unsigned int src_len, const char flag)
{
	struct tm   *newTime;
    time_t      szClock;
	int c, f, p;
	...
	snprintf(src+strlen(src), src_len-strlen(src), "#include <sailing/proto_net.hpp>\n");
	snprintf(src+strlen(src), src_len-strlen(src), "\n");
	snprintf(src+strlen(src), src_len-strlen(src), "#include \"%s.proto.h\"\n", name);
	snprintf(src+strlen(src), src_len-strlen(src), "#include \"%s.%s.hpp\"\n", name, flag=='C'?"clt":"svr");
	...
	for(c=0; c<nmodules_count; c++) {
		...
		for(f=nmodules[c].f_start; f<nmodules[c].f_start+nmodules[c].f_count; f++) {
			...
			snprintf(src+strlen(src), src_len-strlen(src), "static bool %s_%s(bool bTextMode, I%s%s** pHooks, int nHookCount, const void* pData, unsigned int nSize)\n", nmodules[c].name, nfunctions[f].name, nmodules[c].name, pname);
			snprintf(src+strlen(src), src_len-strlen(src), "{\n");
		...
		snprintf(src+strlen(src), src_len-strlen(src), "bool C%s%sParser::Dispatch(const void* pData, unsigned int nSize)\n", nmodules[c].name, pname);
		snprintf(src+strlen(src), src_len-strlen(src), "{\n");
		snprintf(src+strlen(src), src_len-strlen(src), "	if(m_pStream->IsTextMode()) {\n");
		...
		for(f=nmodules[c].f_start; f<nmodules[c].f_start+nmodules[c].f_count; f++) {
			if(nfunctions[f].type==flag) continue;
			snprintf(src+strlen(src), src_len-strlen(src), "void C%s%sStub::%s(", nmodules[c].name, pname, nfunctions[f].name);
			for(p=nfunctions[f].p_start; p<nfunctions[f].p_start+nfunctions[f].p_count; p++) {
			...
			snprintf(src+strlen(src), src_len-strlen(src), "	pSendBuf = m_pStream->GetSendBuf(nSendBufSize);\n");
			snprintf(src+strlen(src), src_len-strlen(src), "	if(m_pStream->IsTextMode()) {\n");
			...
			snprintf(src+strlen(src), src_len-strlen(src), "	}\n");
			snprintf(src+strlen(src), src_len-strlen(src), "	m_pStream->SendData(pSendBuf, (unsigned int)m_nlen);\n");
			snprintf(src+strlen(src), src_len-strlen(src), "}\n");
			snprintf(src+strlen(src), src_len-strlen(src), "\n");
		}
	}
